...
function parentSort(fullList) {
  ...
  function childsToReact(childPrefix, currList) {
    var returnReact = [];
    ...
    for (var i = 0; i < currList.length; i++) {
      ...
      returnReact.push(
          <View key={currList[i]._id}>
            {categoryItem(currList[i], childPrefix)}
            {childsToReact(childPrefix + " -", currList[i].children)}
            </View>
        );
    }
    return returnReact;
                ...
                for (var i = 0; i < categories.length; i++) {
                    ...
                    returnReact.push(
                        <View key={categories[i]._id} style={styles.categoryContainer}>
                            ...
                            {childsToReact("-", categories[i].children)}
                        </View>
                    );
                }
                return returnReact;
...
function categoryItem(currCategory, prefix){
...
<TouchableOpacity onPress={() => {
                                ...
                                {prefix || ""} {currCategory.name} - {currCategory.description}
                                </Text>
                            </TouchableOpacity>
    );
}
